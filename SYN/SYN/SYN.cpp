...
void printInfo() {
    if (listenPort != -1) {
        ...
        if (info.srcport != listenPort && info.dstport != listenPort) {
            //cout << "sourceIP     :" + info.srcIP<<"\t 端口："<<info.srcport<< endl; 
            //cout << "destinationIP:" + info.dstIP<<"\t 端口：" << info.dstport << endl;
            return;
        }
    }
    cout << "===================================数据解析====================================\n" << endl;
    ...
    if (info.flag & 0x10) {
        ...
        //else {
        //    /* 如果是本机发送的ack包，有两种情况，
        //     * 1.本机为接收方，该ack为本机响应ack，同时syn也为1
        //     * 2.本机为请求方，该ack为服务端响应，此时也应该减一，因为这种情况的syn也加了1
        //     */
        //    if (info.flag & 0x02) {
        //        onlySynCount--;
        //    }
    ...
    printf("\n当前无效SYN包数量为：");
    cout << onlySynCount << endl;

   cout << "===============================================================================\n" << endl;
   if (onlySynCount >= MAX_SYN) {
       cout << "****************************************************************************" << endl;
       cout << "                                                                            " << endl;
       cout << "            系统疑似遭到来自 " + info.srcIP + " 的syn洪泛攻击               " << endl;
       cout << "                                                                            " << endl;
       cout << "****************************************************************************\n\n\n" << endl;
...
void ethernet_protocol_packet_callback(u_char* argument, const struct pcap_pkthdr* packet_header, const u_char* packet_content)
{
    ...
    //strcpy(info.srcMac, (char*)ethernet_protocol->ether_shost);
    //strcpy(info.dstMac, (char*)ethernet_protocol->ether_dhost);
...
int main()
{
    ...
    */
    ...
    {
        printf("%d. 网卡名：%s", ++i, d->name);

        //printf("\tIP地址: %s\n", inet_ntoa(((struct sockaddr_in*)d)->sin_addr));
        if (d->description)
            printf(" 描述：(%s)\n", d->description);
        else
    ...
    cout << endl;
    cout << "监听ip:" + listenIp + "..." << endl;
    printf("监听端口:%d", listenPort);
